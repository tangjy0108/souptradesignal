...
// Mobile tabs
type MobileTab = 'chart' | 'signal' | 'alerts';
...
export default function App() {
  ...
  const handleScan = async (symbols: string[]) => {
    ...
    for (const sym of symbols) {
      try {
        ...
        const patterns = detectHarmonics(klines);
        if (patterns.length > 0) results.push({ symbol: sym, pattern: patterns[0] });
      } catch {}
    }
  ...
  const SearchDropdown = ({ inputRef }: { inputRef: React.RefObject<HTMLInputElement> }) => {
    ...
    return (
    <div style={style} className="bg-[#1E222D] border border-[#2A2E39] rounded-xl shadow-2xl overflow-hidden">
  ...
  const RightPanelContent = () => (
    <>
      ...
      <div className="p-4 border-b border-[#2A2E39]">
        ...
        {strategyId !== 'harmonics' && strategyResult ? (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  {STRATEGIES.map(s => (
                    <button key={s.id} onClick={() => setStrategyId(s.id)}
                      ...
                      {s.id === 'ms_ob' ? 'MS+OB' : s.id === 'structural_reversal' ? 'PRZ' : s.id === 'smc_session' ? 'SMC' : '諧波'}
                    </button>
                  ))}
                </div>
                {strategyId === 'harmonics' ? (
